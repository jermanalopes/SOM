...
tic;
dado = load('dados_3.txt');                    %matriz de caracteristicas
...
plot (Classe1(:,4),Classe1(:,5),'r.',Classe2(:,4),Classe2(:,5),'b.',Classe3(:,4),Classe3(:,5),'m.');
legend('ICC','Isquêmica','Saudáveis','Location','NorthEastOutside')
...
for i = 1:Neulinhas,
    for j = 1:Neucolunas,
        if ( (votacao(i,j,1) > votacao (i,j,2)) & (votacao(i,j,1) > votacao (i,j,3)) )
            plot (j,i,'r.');
            Mpesos(i,j,N_Col) = 1;
        elseif ( (votacao(i,j,2) > votacao (i,j,1)) & (votacao(i,j,2) > votacao (i,j,3)) )
            plot (j,i,'b.');
            Mpesos(i,j,N_Col) = 2;
        elseif ( (votacao(i,j,3) > votacao (i,j,1)) & (votacao(i,j,3) > votacao (i,j,2)) )
            plot (j,i,'y.');
            Mpesos(i,j,N_Col) = 3;
        else
            plot (j,i,'w.');
