...
Nepocas = 500;                             
Nvizinhos = 3;                             %numero de vizinhos
...
%% Inicializacao da matriz de neuronios
...
plot (Classe1(:,4),Classe1(:,5),'r.',Classe2(:,4),Classe2(:,5),'b.',Classe3(:,4),Classe3(:,5),'m.',Classe4(:,4),Classe4(:,5),'.g');
legend('ICC','Isquêmica','Saudáveis','Chagas','Location','NorthEastOutside')
...
for i = 1:Neulinhas,
    for j = 1:Neucolunas,
        if ( (votacao(i,j,1) > votacao (i,j,2)) & (votacao(i,j,1) > votacao (i,j,3)) & (votacao(i,j,1) > votacao (i,j,4)))
            plot (j,i,'r.');
            Mpesos(i,j,N_Col) = 1;
        elseif ( (votacao(i,j,2) > votacao (i,j,1)) & (votacao(i,j,2) > votacao (i,j,3)) & (votacao(i,j,2) > votacao (i,j,4)) )
            plot (j,i,'b.');
            Mpesos(i,j,N_Col) = 2;
        elseif ( (votacao(i,j,3) > votacao (i,j,1)) & (votacao(i,j,3) > votacao (i,j,2))& (votacao(i,j,3) > votacao (i,j,4)))
            plot (j,i,'y.');
            Mpesos(i,j,N_Col) = 3;
        elseif ( (votacao(i,j,4) > votacao (i,j,1)) & (votacao(i,j,4) > votacao (i,j,2))& (votacao(i,j,4) > votacao (i,j,3)))
            plot (j,i,'m.');
            Mpesos(i,j,N_Col) = 4;
        else
            plot (j,i,'w.');
